...
class AppDelegate: UIResponder, UIApplicationDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
	...
	func launchWithData(data:AnyObject){
		...
		let rightButton = UIBarButtonItem.init(title: "Upload", style: .Plain, target: self, action: #selector(openImagePicker))
	...
	func imagePickerController(picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : AnyObject]) {
		...
		if(data != nil){
			print("data not nil")
			Fire.shared.uploadImage(data!, completionHandler: { (filename, uploadURL) in
				if(filename != nil){
					print(filename!)
				}
				if(uploadURL != nil){
